...
export module player.view;

// view pitching times
export constexpr float DAMAGE_TIME = 0.5f;
...
=================
*/
export void ClientEndServerFrame(entity &ent);
...
static inline void P_WorldEffects(entity &current_player)
{
	...
	if (current_waterlevel == 3)
	{
		...
		if (breather || envirosuit)
		{
			...
			if (((int32_t)(current_player.client->breather_framenum - level.framenum) % 25) == 0)
			{
...
static void P_DamageFeedback(entity &player)
{
	...
	if (player.client->anim_priority < ANIM_PAIN && player.s.modelindex == MODEL_PLAYER)
	{
		static int32_t i;
...
static inline void G_SetClientFrame(entity &ent)
{
	...
	{
		if ((size_t)ent.s.frame > ent.client->anim_end)
		{
			ent.s.frame--;
			return;
		}
	}
	else if ((size_t)ent.s.frame < ent.client->anim_end)
	{
