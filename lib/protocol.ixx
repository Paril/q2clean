...
export extern "C" struct pmove_state
{
	pmtype	pm_type;
...
export enum cvar_flags : int32_t
{
	CVAR_NONE,
...
// print level
export enum print_level : uint8_t
{
	// pickup messages
	PRINT_LOW,
	// death messages
...
export enum sound_channel : uint8_t
{
	CHAN_AUTO,
	CHAN_WEAPON,
	CHAN_VOICE,
...
export enum sound_attn : uint8_t
{
...
export enum multicast_destination : uint8_t
{
	// send to all
	MULTICAST_ALL,
...
export enum box_edicts_area : uint8_t
{
	AREA_SOLID = 1,
	AREA_TRIGGERS = 2
};
...
export enum surface_flags : uint32_t
{
	SURF_NONE = 0,
...
export constexpr surface null_surface { stringarray<16>({ '\0' }), SURF_NONE, 0 };
...
export constexpr size_t MESSAGE_LIMIT = 1400;

// max size of a Quake path
export constexpr size_t MAX_QPATH = 64;

//
// muzzle flashes / player effects
//
export enum muzzleflash : uint8_t
{
	MZ_BLASTER,
	MZ_MACHINEGUN,
	MZ_SHOTGUN,
...
//
export enum monster_muzzleflash : uint8_t
{
...
// and broadcast.
export enum temp_event : uint8_t
{
	TE_GUNSHOT,
	TE_BLOOD,
	TE_BLASTER,
...
// color for TE_SPLASH
export enum splash_type : uint8_t
{
	SPLASH_UNKNOWN,
	SPLASH_SPARKS,
	SPLASH_BLUE_WATER,
...
export extern "C" struct usercmd
{
	uint8_t				msec;
	button_bits			buttons;
private:
...
export constexpr size_t MAX_TOUCH = 32;

export extern "C" struct pmove_t
{
	// state (in / out)
	pmove_state	s;
...
// per-level limits
//
export constexpr size_t MAX_CLIENTS = 256;	// absolute limit
export constexpr size_t MAX_LIGHTSTYLES = 256;
#ifdef KMQUAKE2_ENGINE_MOD
export constexpr size_t MAX_EDICTS = 8192;	// must change protocol to increase more
export constexpr size_t MAX_MODELS = 8192;	// these are sent over the net as bytes
export constexpr size_t MAX_SOUNDS = 8192;	// so they cannot be blindly increased
export constexpr size_t MAX_IMAGES = 2048;
#else
export constexpr size_t MAX_EDICTS = 1024;	// must change protocol to increase more
export constexpr size_t MAX_MODELS = 256;	// these are sent over the net as bytes
export constexpr size_t MAX_SOUNDS = 256;	// so they cannot be blindly increased
export constexpr size_t MAX_IMAGES = 256;
#endif
export constexpr size_t MAX_ITEMS = 256;
export constexpr size_t MAX_GENERAL = (MAX_CLIENTS * 2); // general config strings
...
//
export enum config_string : int32_t
{
	CS_NAME,
	CS_CDTRACK,
	CS_SKY,
...
export constexpr size_t CS_SIZE(const config_string &cs)
{
...
export enum entity_effects : uint32_t
{
	EF_NONE,
...
export enum render_effects : uint32_t
{
...
export enum entity_event : uint32_t
{
	EV_NONE,
	EV_ITEM_RESPAWN,
	EV_FOOTSTEP,
...
export constexpr size_t MAX_ENT_CLUSTERS = 16;

// entity server flags; the engine uses these, so don't change this.
export enum server_flags : uint32_t
{
	SVF_NONE,
...
export extern entityref world;

// player_state_t->refdef flags
export enum refdef_flags
{
	RDF_NONE,
